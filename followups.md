...
| Vercel Production自動deploy接続の現在状態 | 旧CoWork記録上は、GitHub `master` pushがVercel Production自動deployに接続されていた可能性が高い。ただし、Vercel Dashboard実画面、現在のProduction Branch、Codex移行後pushで実際にdeployが走ったかは未確認。 | Vercel project名の正式表記、GitHub / Vercel接続の現在状態、Production Branch、Deployment Source、preview deployの実運用有無。 | Vercel Dashboard実画面read-only確認GO またはpush前の人間確認 | Vercel設定変更、deploy実行、本番URLアクセス、本番確認、`.env*` や環境変数値確認へ進まない。push後も本番反映確認・本番実機確認は別GOにする。 | CoWork聞き取り結果、IMP-12自動deploy不起動履歴 |
